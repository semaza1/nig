...
/* ── Helper: safe query wrapper – never crashes ─────────────── */
...
/* ── Helper: unpaid principal for one loan ──────────────────── */
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    /* ── Full profile (view popup) ─────────────────────────── */
    if (isset($_GET['id'], $_GET['full']) && $_GET['full'] === '1') {
        $id = (int)$_GET['id'];

        // Base user
        $user = db_query_one($mysqli, "
        ...
        foreach ($loan_rows as $r) {
            $r['unpaid_principal'] = round(
                loan_unpaid($mysqli, (int)$r['loan_id'], (float)$r['principal'], (string)$r['status']), 2
            );
            $loans[] = $r;
        }

        // Loans this user has guaranteed
        // Check if loan_guaranters table exists first
        $guaranteed = [];
        ...
        // Assets held
        // Check if asset_holders table exists first
        $assets = [];
    ...
    $where    = '';
    if ($q !== '') {
        $e     = $mysqli->real_escape_string($q);
        $where = " WHERE names LIKE '%$e%' OR email LIKE '%$e%' OR phone1 LIKE '%$e%' OR phone2 LIKE '%$e%' OR nid_passport LIKE '%$e%'";
    }
    $total  = (int)$mysqli->query("SELECT COUNT(*) FROM users$where")->fetch_row()[0];
    $offset = ($page - 1) * $per_page;
    $res    = $mysqli->query("
        SELECT id, names, nid_passport, email, phone1, phone2,
               guarantee_name, guarantee_nid_passport, guarantee_email,
               guarantee_phone1, guarantee_phone2, is_member, is_admin
        FROM users$where ORDER BY id DESC LIMIT $offset,$per_page
    ");
    $rows = [];
...
if ($action === 'create') {
    ...
    $stmt = $mysqli->prepare("
        INSERT INTO users (names,nid_passport,email,password,phone1,phone2,
            guarantee_name,guarantee_nid_passport,guarantee_email,
            guarantee_phone1,guarantee_phone2,is_member,is_admin)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
    ");
    ...
    if ($stmt->execute()) {
        ...
        $row = db_query_one($mysqli, "SELECT id,names,nid_passport,email,phone1,phone2,guarantee_name,guarantee_nid_passport,guarantee_email,guarantee_phone1,guarantee_phone2,is_member,is_admin FROM users WHERE id=$newid");
        echo json_encode(['success'=>true,'data'=>$row]);
    } else {
...
if ($action === 'update') {
    ...
    if ($stmt->execute()) {
        ...
        $row = db_query_one($mysqli, "SELECT id,names,nid_passport,email,phone1,phone2,guarantee_name,guarantee_nid_passport,guarantee_email,guarantee_phone1,guarantee_phone2,is_member,is_admin FROM users WHERE id=$id");
        echo json_encode(['success'=>true,'data'=>$row]);
    } else {
        echo json_encode(['success'=>false,'message'=>$mysqli->error]);
    }
